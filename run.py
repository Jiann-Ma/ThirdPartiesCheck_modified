...
from utils import judicialRecordsCheck
...
class MyApp(object):
    ...
    def check(self):
        ...
        #TODO 提取參數以及進行驗證
        self.file_path = self.input_file_path.get().strip()
    ...
    def run(self):
        ...
        try:
            ...
            if self.check():
                # 把utils的class導進來，survey是自己命名的，參數是使用者要打的東西
                self.survey = judicialRecordsCheck(self.input_file_path, state, self.logger)
                self.survey.main()
            else:
